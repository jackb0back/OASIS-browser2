...
function action(a) {
  ...
  if (value === '') {
    ...
    let b64URL = getUrl();
    document.getElementById(getActiveFrameId()).src = "" + getUrl();
  } else {
    let b64URL = getUrl();
    document.getElementById(getActiveFrameId()).src = "" + getUrl();
  }
...
document.querySelector('button[data-add-tab]').addEventListener('click', _ => {
  ...
  frame.setAttribute('src', 'oasis.html');
...
function WebU() {
   ...
   // _.preventDefault()
    ...
    var uwu = i++;
    var frame = document.createElement('IFRAME');
    frame.setAttribute('src', 'https://webublock1.glitch.me');
    frame.setAttribute('allow', 'fullscreen');
    frame.setAttribute('sandbox', 'allow-same-origin allow-scripts allow-popups allow-forms allow-pointer-lock')
    document.body.appendChild(frame);
    frame.setAttribute('class', 'iframething');
...
document.getElementById('B').innerHTML = ` <button id="webu" onclick="WebU()">
new browser
</button>`